...
/// <summary>
/// Cliente para interagir com a API local compatível com o contrato MyAnimeList.
/// </summary>
public class MyAnimeListImportClient(HttpClient httpClient)
{
    ...
    {
        using var response = await _httpClient.GetAsync($"ApiMyAnimeList/{malId}", cancellationToken);
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new HttpRequestException(
                $"Falha ao consultar ApiMyAnimeList. StatusCode={(int)response.StatusCode}",
                null,
                response.StatusCode);
        }
        var payload = await response.Content.ReadFromJsonAsync<MyAnimeListAnimeDetailsResponseDto>(cancellationToken: cancellationToken);
...
/// <summary>
/// Representa os dados de importação de um anime obtidos da API MyAnimeList.
/// </summary>
...
file class MyAnimeListAnimeDetailsResponseDto
{
    public int MalId { get; set; }
    public string? Url { get; set; }
    public MyAnimeListImageCollectionDto? Images { get; set; }
    public string? Trailer { get; set; }
    public bool Approved { get; set; }
    public string? Title { get; set; }
    public string? TitleEnglish { get; set; }
...
file class MyAnimeListImageCollectionDto
{
    public MyAnimeListImageVariantDto? Jpg { get; set; }
}

file class MyAnimeListImageVariantDto
{
